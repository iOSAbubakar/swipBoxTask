...
//	}
    ...
    func open(dbPath: String, copyFile: Bool = false, inMemory: Bool = false) -> Bool {
        ...
        if inMemory {
            ...
            if copyFile {
                // Old DB location
                guard var docDir = fm.urls(for: .documentDirectory, in: .userDomainMask).first else { return false }
// If macOS, add app name to path
                ...
                // New destination
                guard var libDir = fm.urls(for: .libraryDirectory, in: .userDomainMask).first else { return false }
// If macOS, add app name to path
#if os(OSX)
